<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST" && isset($_POST["submit"]))
    {
        ...
        if(empty($fnameErr) && empty($emailErr) && empty($pwdErr) && empty($cpwdErr) && empty($pwdMatchErr))
        {
            ...
            if($stmt->execute())
            {
                header("Location: signup.php?update=success");
                exit;
            }
            else
            {
...
<body>
    <!-- Main container -->
    <div class="d-flex flex-column container-lg justify-content-center align-items-center border border-5 border-dark rounded w-50 mt-5 mb-5">
        ...
        <p class="fst-italic">Already have an account? <a href="#">Login here</a></p>
    </div>
    <!-- Bootstrap JS -->
